...
class SendResult extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      color: "#B7F072",
      isVisible: true,
      result_type : props.result_type?props.result_type:'result_reward'
    };
  ...
  render() {
    ...
    let title = 'SE ENVIO A';
    if(this.state.result_type=='result_reward')
      title = 'SE RECOMPENSÓ A:';
    if(this.state.result_type==config.SA_RESET_BALANCE)
      title = 'SE RETORNÓ A:';
    if(this.state.result_type==config.SA_SEND_EXTRA_BALANCE)
      title = 'SE RETORNÓ A:';
    ...
    return (
      ...
      <View style={styles.container}>
        <View style={{padding: 20, flex:1, backgroundColor:'transparent', paddingLeft:30, paddingRight:30, alignItems:'center', flexDirection:'column', justifyContent:'center'}}>
          ...
          <View style={{position:'absolute', right:10, bottom: 10, flexDirection:'row', justifyContent: 'flex-end', backgroundColor:'transparent'}}>
            <Icon
              raised
              containerStyle={{backgroundColor:'#ff7233', borderWidth: 0.5, borderColor: 'transparent' }}
              name='md-checkmark'
              type='ionicon'
              color='#ffffff'
              underlayColor='#ff9e5d'
