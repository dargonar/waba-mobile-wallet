...
// 	}
  	...
  	//   console.log(' ----------------------- History::_onPressButton');
		...
		if(rowData.__typename == 'Transfer' && config.ASSET_ID == rowData.amount.asset.id)
		{
			this.props.navigator.push({
				...
				title: 'Detalles',
				passProps: {...rowData, rowData:rowData, account_name:this.props.account.name}
			});
			return;
		}
		ToastAndroid.show('Esta transacción no tiene detalles que mostrar', ToastAndroid.LONG);		
  }
  ...
  _renderRow(rowData, sectionID, rowID) {
    ...
    if(rowData.__typename == 'WithdrawPermissionClaim' && this.state.is_subaccount) {
    	console.log('***************************************', rowData.__typename);
      // let icon 						= 'md-card';
      let bg     					= {credit_request:'#29c3cb'};
      let fecha  			    = RawTxHelper._getFechaFromUTC(rowData.block.timestamp);
      ...
      return(
        <TouchableHighlight style={styles.txRow} underlayColor={'#ccc'} onPress={() => { this._onPressButton(rowID, rowData)}}>
          <View style={styles.row_container}>
            <View style={{width:60, backgroundColor:bg['credit_request'], borderBottomLeftRadius: 7,borderTopLeftRadius: 7}}>
            	<View style={styles.row_avatar}>
            		<Image source={iconsMap['md-arrow-up']} style={styles.row_arrow}/>
            	</View>
            </View>
            
            <View style={styles.row_content}>
							<View style={styles.row_line1}>
								<Text style={styles.row_action}>INICIO DE CAJA</Text>
							</View>
							<View style={styles.row_line2}>
								<Text style={styles.row_dea}>Administración</Text>
							</View>
							<View style={styles.row_line2}>
    ...
    if(rowData.__typename == 'WithdrawPermission' && this.state.is_subaccount) {
      ...
      return(
        <TouchableHighlight style={[styles.txRow, {marginBottom:30}]} underlayColor={'#0f0'} onPress={() => { this._onPressButton(rowID, rowData)}}>
          <View style={styles.row_container}>
          	<View style={{width:60, backgroundColor:bg['credit_request'], borderBottomLeftRadius: 7,borderTopLeftRadius: 7}}>
            	<View style={[styles.row_avatar, {borderWidth: 0.5, borderColor:bg['credit_request']}]}>
	              <Image source={iconsMap[icon]} style={styles.row_card}/>
	            </View>
            </View>
            
            <View style={styles.row_content}>
							<View style={styles.row_line1}>
								<Text style={styles.row_action}>ASIGNACION COMO SUBCUENTA</Text>
							</View>
							<View style={styles.row_line2}>
								<Text style={styles.row_dea}>{rowData.from.name}</Text>
							</View>
							<View style={styles.row_line2}>
								<Text style={styles.row_hour_item}>{hora}{" - "}{fecha}</Text>
							</View>
    	...
    	let txData = RawTxHelper.getTxData(rowData, this.props.account.name);
      ...
      return (
        <TouchableHighlight style={styles.txRow} underlayColor={'#ccc'} onPress={() => { this._onPressButton(rowID, rowData)}}>
          <View style={styles.row_container}>
            <View style={{width:60, backgroundColor:RawTxHelper.bg[txData._bg], borderBottomLeftRadius: 7,borderTopLeftRadius: 7}}>
            	<View style={styles.row_avatar}>
            		<Image source={iconsMap['md-arrow-up']} style={[styles.row_arrow, {transform : [{rotate: RawTxHelper.rotato[txData._rotato]}]}]}/>
            	</View>
            </View>
            <View style={styles.row_content}>
							<View style={styles.row_line1}>
							...
							<View style={styles.row_line2}>
								<Text style={styles.row_hour_item}>
										{txData._hora}{" - "}{ txData._fecha}
								</Text>
							</View>
            </View>
            <View style={styles.col_amount}>
