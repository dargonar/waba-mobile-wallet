...
// 	}
  ...
  _renderRow(rowData, sectionID, rowID) {
    ...
    if(rowData.__typename == 'WithdrawPermissionClaim' && this.state.is_subaccount) {
      ...
      let fecha  			    = TxHelper._getFecha(rowData.block.timestamp);
      let hora   			    = TxHelper._getHora(rowData.block.timestamp);
    ...
    if(rowData.__typename == 'WithdrawPermission' && this.state.is_subaccount) {
      ...
      return(
		...
		if(rowData.__typename == 'CreditRequest') {
      	...
      	let txData = TxHelper.getTxData(rowData, this.props.account.name);
        ...
        if(rowData.message)
	      ...
	      return (
	        <TouchableHighlight style={styles.txRow} underlayColor={'#ccc'} onPress={() => { this._onPressButton(rowID, rowData)}}>
	          <View style={styles.row_container}>
	            <View style={{width:60, backgroundColor:TxHelper.bg[txData._bg], borderBottomLeftRadius: 7,borderTopLeftRadius: 7}}>
	            	<View style={styles.row_avatar}>
	            		<Image source={iconsMap['md-arrow-up']} style={[styles.row_arrow, {transform : [{rotate: TxHelper.rotato[txData._rotato]}]}]}/>
	            	</View>
	            </View>
	            <View style={styles.row_content}>
								<View style={styles.row_line1}>
									<Text style={styles.row_action}>{TxHelper.action[txData._action]}</Text>
								</View>
								<View style={styles.row_line2}>
